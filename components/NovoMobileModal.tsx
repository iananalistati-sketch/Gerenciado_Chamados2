...
export default function NovoMobileModal({
  ...
  const { appOptions, targetVersions } = useMemo(() => {
    ...
    if (appIdx !== -1) {
      mobileConfig.slice(1).forEach((row) => {
        ...
        if (!result.some((item) => normalize(item) === normalize(app))) {
          result.push(app);
        }
        if (targetIdx !== -1) {
  ...
  useEffect(() => {
    ...
    if (dataAtualizacaoHeader) {
      initialData[dataAtualizacaoHeader] = getToday();
    }
    if (responsavelAtualizacaoHeader) {
      initialData[responsavelAtualizacaoHeader] = currentUserName;
    }
    if (statusAtualizacaoHeader) {
      initialData[statusAtualizacaoHeader] = "SEM INFORMAÇÃO";
    }
    ...
    setFormData((previous) => {
      ...
      if (header === setorHeader && setorLocalizadoHeader) {
        const currentLocation = String(
          previous[setorLocalizadoHeader] || ""
        ).trim();
        if (!currentLocation) {
          next[setorLocalizadoHeader] = value;
        }
      }
      ...
      if (contadorHeader && (header === appHeader || header === versaoHeader)) {
        ...
        next[contadorHeader] =
          normalize(app) !== "todos" && version.trim() ? "1" : "0";
      }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (status === "A") {
      const activeCollectorExists = allRows.some((row) => {
        ...
        const existingStatus = String(row[statusIdx] || "")
          .trim()
          .toUpperCase();
        return (
    ...
    if (dataAtualizacaoHeader) {
      preparedData[dataAtualizacaoHeader] = getToday();
    }
    if (responsavelAtualizacaoHeader) {
      preparedData[responsavelAtualizacaoHeader] = currentUserName;
    }
    if (contadorHeader) {
      preparedData[contadorHeader] =
        normalize(app) !== "todos" && version ? "1" : "0";
    }
    if (statusAtualizacaoHeader) {
      preparedData[statusAtualizacaoHeader] =
        calculateUpdateStatus(app, version);
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {renderField(setorLocalizadoHeader, "Setor localizado", {
              ...
              required: true,
              choices: setores.map((item) => ({ value: item, label: item })),
              helper: "Ao selecionar o Setor, ele será sugerido automaticamente aqui.",
            })}
            ...
            {renderField(versaoHeader, "Versão", {
              ...
              helper: isTodos
                ...
                : "Obrigatória quando o equipamento utiliza um App específico.",
            })}
